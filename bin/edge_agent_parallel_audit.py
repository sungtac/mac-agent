...
def audit(manager: WorktreeManager, repo_root: str | Path) -> list[AuditFinding]:
...
def audit_payload(manager: WorktreeManager, repo_root: str | Path) -> dict:
    findings = audit(manager, repo_root)
    return {
...
def main() -> int:
    ...
    payload = audit_payload(manager, args.repo)
