...
상태: Discord 연동 절은 2026-08-02 퇴역한 historical 기록이다. 현재 active 경로는
Discord bot/fallback을 호출하지 않는다.
...
호스트 `verify-task-orchestrator.py`의 역할 고정(Claude=경량 구현/전체 독립 리뷰, Antigravity=조사·독립검증, Codex=계획·실행)은 이 정책과 무관 — 사용량과 상관없이 그대로 유지(자기평가 방지 설계를 사용량 이유로 깨면 안 됨). 구형 JS Workflow 경로는 제거되어 이 정책의 진입점이 아니다.
