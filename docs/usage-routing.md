...
## 기반 규칙 (사용자 확정, 2026-07-26)

**Rule A — Orchestrator 내부 추론 우선.** 이 살아있는 세션(클로드) 고유의 판단이 필요한 작업은 아래 라우팅 순서와 무관하게 클로드가 그대로 진행한다. 단, 클로드 자체 사용량이 `coach` 기준 `red`면 범위를 최소화한다. 셸 스크립트가 "클로드가 처리"를 대신 실행할 방법이 없다는 것과 같은 이야기 — 이 규칙은 애초에 라우팅 대상이 아닌 작업을 가려내는 경계선이다.

**Rule B — 안티그래비티는 명시적 트리거 시만.** 안티그래비티(gemini 백엔드)는 아무 작업에나 기본값으로 안 쓴다. `route-dispatch.sh` 호출 자체가 그 "명시적 트리거"다 — 즉 이미 "위임 가능하고 단순함"으로 판단된 작업에 한해서만 트리거가 성립한다. Rule A 영역(오케스트레이터 고유 판단)에는 애초에 적용되지 않으므로 Rule A와 충돌하지 않는다.
...
4. **claude-main/Orchestrator 고유 판단 작업**: Rule A 그대로 — 이 순서와 무관하게 클로드가 진행, red면 범위 최소화.
...
- **4번(Rule A)은 강제화 불가**: "이건 오케스트레이터 고유 판단이 필요한 작업이다"는 판단 자체가 의미론적 판단이라 셸 스크립트가 검증 못함. `usage-routing-check.sh`(Stop 훅)가 사후에 "클로드 사용량이 낮았는데 라우팅 흔적이 없다"를 기계적으로 감지해서 1회 지적하는 게 유일한 backstop — verify-task-stop-check.sh와 같은 성격의 근사치다.
