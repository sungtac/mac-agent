...
- **날씨 도구(`20-get_short_term_forcast`)의 `nx`/`ny`는 검색으로 확인이 안 됨** — 광주광역시 격자좌표를 공식 문서/검색으로 못 찾아서, 통용되는 값(nx=58, ny=74)으로 실제 호출해보고 7월 말 기온이 33~35도로 실제 여름 날씨와 부합하는지로 간접 검증함.
