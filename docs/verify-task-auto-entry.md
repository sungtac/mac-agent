...
2. 코드 변경 의도이면 `~/.claude/hooks-state/verify-task-v2/<session_id>.json`에 상태를 기록하고 Workflow 호출 지시를 추가한다.
3. `PreToolUse(Edit|Write)`의 `verify-task-pre-edit-gate.sh`가 메인 세션의 직접 편집을 차단한다.
4. Claude가 `Workflow({scriptPath: "~/.claude/workflows/verify-task-v2.js", ...})`를 호출한다.
5. `PostToolUse(Workflow)`의 `verify-task-post-tool-check.sh`가 `finalVerdict.passed === true`인 경우에만 `workflow_completed`로 전환한다.
...
- Workflow의 `args.cwd`가 현재 세션의 작업 디렉토리와 다르거나 `args.task`가 비어 있으면 성공 결과를 상태에 반영하지 않는다.
- Workflow 호출만으로는 통과시키지 않는다. 결과를 해석할 수 없거나 `passed`가 아니면 `workflow_failed`로 닫힌 상태를 유지한다.
