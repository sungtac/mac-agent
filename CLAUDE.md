...
- `workflows/verify-task-v2.js` — the only verification Workflow: deterministic file-based handoff and light/full multi-agent review plus opt-in `nanoMode`. Claude is limited to light implementation or full independent review; Codex owns full planning/execution; Antigravity investigates and independently reviews. The harness lives in `bin/verify-task-harness.py`. → [docs/verify-task-v2-design.md](docs/verify-task-v2-design.md)
