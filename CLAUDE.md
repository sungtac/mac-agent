...
- `workflows/verify-task-v2.js` — tiered (light/full) multi-agent review plus opt-in `nanoMode` contract-step loop. Light/full behavior stays backward-compatible; nano mode adds plan → execute → event/idempotency check → light validation → risk-based mid/full integration review. Findings accumulate permanently into `docs/codex-harness.md`. → [docs/verify-task-v2-design.md](docs/verify-task-v2-design.md)
