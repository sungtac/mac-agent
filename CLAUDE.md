...
- `workflows/verify-task-v2.js` — the only verification Workflow: tiered (light/full) multi-agent review plus opt-in `nanoMode` contract-step loop. Light/full behavior stays backward-compatible; nano mode adds plan → execute → event/idempotency check → light validation → risk-based mid/full integration review. Findings accumulate permanently into `docs/codex-harness.md`. → [docs/verify-task-v2-design.md](docs/verify-task-v2-design.md)
...
- `bin/discord-bot.py` — persistent (launchd `KeepAlive`) Discord bot: `!주간보고서`/`!상태` on-demand triggers, work-log/verify-task-v2 reply-triggered retries, and Phase 3 free-form `claude -p` chat relay (`FREE_CHAT_*`, owner-only). `bin/codex-bot.py` — separate bot process/identity for Codex-related commands (`!코덱스`, `!코덱스대화`, `!코덱스대화초기화`, `!코덱스중지`), split out from discord-bot.py 2026-07-29. Both share `bin/discord_bot_common.py` (subprocess env, usage gate, process-group kill helpers). `bin/discord-notify.sh` — one-way fire-and-forget notify helper (used by weekly-report.sh, work-log-stop-check.sh, verify-task-v2.js escalations), works independently of either bot process. → [docs/discord-bot.md](docs/discord-bot.md)
