#!/usr/bin/env bash
# Runs every Friday via launchd (~/Library/LaunchAgents/com.macagent.weekly-report.plist)
...
매주 금요일에 실행되는 주간 업무 보고서 작성 작업입니다. 이번 주 범위: ${MON} ~ ${FRI}.
...
5. 마지막에 저장한 파일의 절대 경로를 한 줄로 출력하세요.
